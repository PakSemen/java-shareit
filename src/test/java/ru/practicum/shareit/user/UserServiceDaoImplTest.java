...
class UserServiceDaoImplTest {
    ...
    void createUserWithAlreadyExistsExceptionValidationTest() {
        when(userRepository.save(any())).thenThrow(DataAlreadyExistException.class);

        assertThatThrownBy(() -> userService.createUser(userDto)).isInstanceOf(DataAlreadyExistException.class);
    }
    ...
    void updateUserByIdWithAlreadyExistsExceptionValidationTest() {
        UserDto userDtoThird = new UserDto(999L, "User_name", "user@test.testz");
        when(userRepository.findById(userDtoThird.getId())).thenThrow(DataAlreadyExistException.class);
        Long userId = 999L;
        assertThatThrownBy(() -> userService.updateUserById(userId, userDto)).isInstanceOf(DataAlreadyExistException.class);
    }
